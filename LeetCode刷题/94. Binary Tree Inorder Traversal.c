...
/*
// 中序遍历
void inorder(struct TreeNode* node, int* travel, int* returnSize){
    // 遍历到树叶
    if(node == NULL) return;
    // 优先遍历左子树
    inorder(node -> left, travel, returnSize);
    // 保存节点值并更新returnSize
    travel[(*returnSize)++] = node -> val;
    inorder(node -> right, travel, returnSize);
}
...
int* inorderTraversal(struct TreeNode* root, int* returnSize){
    *returnSize = 0;
    ...
    inorder(root, travel, returnSize);
    return travel;
}
*/
// 迭代，利用栈存储节点实现中序遍历
